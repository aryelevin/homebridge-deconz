  ...
  async heartbeat (beat) {
    try {
      ...
      if ((!this.allPanels.length && Object.keys(this.panelsToResources).length) || (beat % updateWeatherEvery === 0 && this.allPanels.length)) {
        ...
        var casigningcert = fs.readFileSync('/Users/levinshome/Downloads/rimon.pem')
        https.get('https://api.open-meteo.com/v1/forecast?latitude=32.08934&longitude=34.83760&hourly=relativehumidity_2m,uv_index&current_weather=true&forecast_days=1', {ca: casigningcert}, res => {
          const data = []
          ...
          res.on('end', () => {
            ...
            if (parsedData) {
              ...
              const humidity = parsedData.hourly.relativehumidity_2m[parsedData.hourly.time.indexOf(parsedData.current_weather.time)] || 0
