...
class DeconzPlatform extends Platform {
  ...
  async heartbeat (beat) {
    try {
      ...
      const updateWeatherEvery = 900 // Seconds!!!
      if (beat % updateWeatherEvery === 0) {
        const tempertaure = 0
        const humidity = 0
        const weather = 0
        const uvIndex = 0
        ...
        for (let index = 0; index < panelsResourcePaths.length; index++) {
          ...
          const gateway = this.gatewayMap[panelsResourcePath[1]]
          gateway.sendStateToPanel(panelsResourcePath, /*gateway.id*/'54ef441000069a5e', '0d020055', weather.toString(16).padStart(8, '0'))
          gateway.sendStateToPanel(panelsResourcePath, /*gateway.id*/'54ef441000069a5e', '00040055', gateway.getAqaraHexFromInt(tempertaure).toString(16).padStart(4, '0') + '0000')
          gateway.sendStateToPanel(panelsResourcePath, /*gateway.id*/'54ef441000069a5e', '00050055', gateway.getAqaraHexFromInt(humidity).toString(16).padStart(4, '0') + '0000')
          gateway.sendStateToPanel(panelsResourcePath, /*gateway.id*/'54ef441000069a5e', '00060055', gateway.getAqaraHexFromInt(uvIndex).toString(16).padStart(4, '0') + '0000')
        }
      }
