...
class DeconzPlatform extends Platform {
  ...
  async init () {
    ...
    if (!this._configJson.homeassistantMode && this._configJson.addShabbatModeDummy === true) {
  ...
  async heartbeat (beat) {
    try {
      ...
      // Added by me: Arye Levin
      if (!this._configJson.homeassistantMode) {
              ...
              try {
                ...
                if (parsedData) {
                  if (!parsedData.error) {
                    ...
                    for (let index = 0; index < that.allPanels.length; index++) {
                      ...
                      if (gateway.initialised) {
                        ...
                        if (temperature !== that.lastWeatherData.temperature) {
                          gateway.sendFeelPageDataToPanel(panelsResourcePath, gateway.id.toLowerCase(), '00040055', gateway.getHexFromFloat32Bit(temperature))
                        }
                        if (humidity !== that.lastWeatherData.humidity) {
                          gateway.sendFeelPageDataToPanel(panelsResourcePath, gateway.id.toLowerCase(), '00050055', gateway.getHexFromFloat32Bit(humidity))
                        }
                        if (uvindex !== that.lastWeatherData.uvindex) {
                          gateway.sendFeelPageDataToPanel(panelsResourcePath, gateway.id.toLowerCase(), '00060055', gateway.getHexFromFloat32Bit(uvindex))
                        }
                      }
                    }
                    ...
                    that.error('Weather data fetch error, reason: ' + parsedData.reason)
                  }
                }
              } catch (error) {
                that.error(error)
