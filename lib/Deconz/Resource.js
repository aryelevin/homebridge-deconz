...
class Resource {
  ...
  constructor (gateway, rtype, rid, body) {
    ...
    if (
      ...
      let { mac, endpoint, cluster } = ApiClient.parseUniqueid(body.uniqueid)

      if (gateway.platform._configJson.separateAccessoryPerEndpoint?.includes("/" + gateway.id + "/" + rtype + "/" + rid)) {
        mac = mac + "_" + endpoint
      }
  ...
  get _serviceName () {
    if (this.rtype === 'groups') {
      ...
      const type = /^(CLIP|ZHA|ZGP)?([A-Za-z0-9]*)$/.exec(this.body.type)?.[2] // 0-9 Added by me: Arye Levin
      switch (type) {
        ...
        case 'AqaraS1ScenePanelSwitch': return 'Label' // Added by me: Arye Levin
        case 'Spectral': return ''
        case 'Switch': return 'Label'
        case 'Temperature': return 'Temperature'
        case 'Thermostat': return /* Added by me: Arye Levin -> */ ((this.body.config.coolsetpoint !== undefined && this.body.config.fanmode !== undefined) || (this.body.config.externalsensortemp !== undefined && this.body.config.speed !== undefined) /* <- This is for the Aqara VRF controller integration..*/)  ? 'HeaterCooler' : /* <- End of Added by me: Arye Levin */ 'Thermostat'
        case 'Time': return ''
  ...
  * @param {DeconzAccessory.Gateway} gateway - The gateway.
  ...
  patchLight (gateway) {
    switch (this.manufacturer) {
      // Added by me: Arye Levin
      case 'Sunricher':
        if (this.model === 'RGB-CCT') {
          this.body.capabilities.color.ct.max = 500 // The range to use in HK (for the Adaptive lighting to work properly [and in sync with other lights in the house]).
          this.body.capabilities.color.ct.originalMax = 450 // The driver logical max CT.
          this.body.capabilities.color.ct.actualMax = 370 // the actual light LED chip max CT.
        }
        break
      // End of Added by me: Arye Levin
      case 'GLEDOPTO':
    ...
    // Added by me: Arye Levin
    const resoucreConfig = gateway.platform._configJson.resourcesCCTRanges?.['/' + gateway.id + this.rpath]
    if (resoucreConfig) {
      if (resoucreConfig.maxHKCt) {
        this.body.capabilities.color.ct.max = resoucreConfig.maxHKCt
        if (resoucreConfig.deviceLogicMaxCT) {
          this.body.capabilities.color.ct.originalMax = resoucreConfig.deviceLogicMaxCT
        }
        if (resoucreConfig.actualLightMaxCt) {
          this.body.capabilities.color.ct.actualMax = resoucreConfig.actualLightMaxCt
        }
      }
      if (resoucreConfig.minHKCt) {
        this.body.capabilities.color.ct.min = resoucreConfig.minHKCt
        if (resoucreConfig.deviceLogicMinCT) {
          this.body.capabilities.color.ct.originalMin = resoucreConfig.deviceLogicMinCT
        }
        if (resoucreConfig.actualLightMinCt) {
          this.body.capabilities.color.ct.actualMin = resoucreConfig.actualLightMinCt
        }
      }
    }
    // End of Added by me: Arye Levin
      ...
      case 'LUMI':
        switch (this.model) {
          ...
          case 'lumi.sensor_switch.aq3': // Xiaomi Aqara smart wireless switch with gyro
            ...
            break
          // Added by me: Arye Levin
          case 'lumi.switch.n4acn4': // Xiaomi Aqara S1 Scene Panel
            buttons.push([1, 'Scene 1', SINGLE])
            buttons.push([2, 'Scene 2', SINGLE])
            buttons.push([3, 'Scene 3', SINGLE])
            buttons.push([4, 'Scene 4', SINGLE])
            buttons.push([5, 'Scene 5', SINGLE])
            buttons.push([6, 'Scene 6', SINGLE])
            break
          // End of Added by me: Arye Levin
          default:
            break
