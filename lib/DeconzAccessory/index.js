...
class DeconzAccessory extends AccessoryDelegate {
  ...
  static get Platform () { return require('./Platform') }
  static get DummySwitch () { return require('./DummySwitch') }
  static get JewishCalendarSensor () { return require('./JewishCalendarSensor') }
    ...
    this
      .on('polled', (device) => {
        ...
        for (const subtype in device.resourceBySubtype) {
          ...
          if (service == null) {
            if (resource.model === 'lumi.switch.n4acn4' && this.platform._configJson.actionsConfigData?.[this.gateway.id]?.aqara_S1_panels?.[resource.rpath]?.expose === false) {
              continue
            }
