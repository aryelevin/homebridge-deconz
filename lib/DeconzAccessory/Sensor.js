...
class Sensor extends DeconzAccessory {
  constructor (gateway, device) {
    ...
    for (const subtype in device.resourceBySubtype) {
      ...
      if (subtype === device.primary/* Added by me: Arye Levin -> */ || (resource.model === 'lumi.switch.n4acn4' && this.platform._configJson.actionsConfigData?.[this.gateway.id]?.aqara_S1_panels?.[resource.rpath]?.expose === false)/* <- End of Added by me: Arye Levin */) {
        continue
      }
