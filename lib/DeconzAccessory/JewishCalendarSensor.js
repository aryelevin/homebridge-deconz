...
import '../DeconzService/JewishCalendarSensor.js'
...
class JewishCalendarSensor extends AccessoryDelegate {
  ...
  constructor (platform, jewishCalendarConfig) {
    ...
    this.services.SefiratHaOmerMourning = new DeconzService.JewishCalendarSensor(this, {name: "Sefirat HaOmer Mourning", subtype: "SefiratHaOmerMourning"});
    this.services.SefiratHaOmer = new DeconzService.JewishCalendarSensor(this, {name: "Sefirat HaOmer", subtype: "SefiratHaOmer"});
    this.services.Mourning = new DeconzService.JewishCalendarSensor(this, {name: "Mourning", subtype: "Mourning"});
    this.services.Purim = new DeconzService.JewishCalendarSensor(this, {name: "Purim", subtype: "Purim"});
    this.services.ShushanPurim = new DeconzService.JewishCalendarSensor(this, {name: "Shushan Purim", subtype: "ShushanPurim"});
    this.services.PurimMeshulash = new DeconzService.JewishCalendarSensor(this, {name: "Purim Meshulash", subtype: "PurimMeshulash"});
    this.services.ShvihiShelPesach = new DeconzService.JewishCalendarSensor(this, {name: "Shvihi Shel Pesach", subtype: "ShvihiShelPesach"});
    this.services.LeapYear = new DeconzService.JewishCalendarSensor(this, {name: "Leap Year", subtype: "LeapYear"});
  ...
  updateSensors() {
    ...
    this.services.SefiratHaOmerMourning.values.contact = this.isSefiratHaOmerMourning();
    this.services.SefiratHaOmer.values.contact = this.isSefiratHaOmer();
    this.services.Mourning.values.contact = this.isMourning();
    this.services.Purim.values.contact = this.isPurim();
    this.services.ShushanPurim.values.contact = this.isShushanPurim();
    this.services.PurimMeshulash.values.contact = this.isPurimMeshulash();
    this.services.ShvihiShelPesach.values.contact = this.isShvihiShelPesach();
    this.services.LeapYear.values.contact = this.isLeapYear();
  }
  ...
  updateJewishDay() {
    ...
    var suntimes = SunCalc.getTimes(midday, this.lat, this.long);
...
*/
  ...
  isThreeWeeks() {
    ...
    var Av9 = new HeDate(this.hDate.getFullYear(), this.hebrewMonths.Av, 9);
  ...
  isMourning() { return this.isSefiratHaOmerMourning() || this.isThreeWeeks();}
  ...
  isChanukah() {
    var ChanukahEnd = new HeDate(this.hDate.getFullYear(), 2, 32);
  ...
  isPurim() {
    ...
    var begin = {month: this.hebrewMonths.Adar2, date: 13};
    var end = {month: this.hebrewMonths.Adar2, date: 14};
    return this._inHebrewHolidayDateRange(begin, end);
  }
  isShushanPurim() {
    // Leap years can make Adar2's month number "bounce" so we check for it
    var isPurimMeshulash = this.isPurimMeshulash() ? 1 : 0;
    var begin = {month: this.hebrewMonths.Adar2, date: 14 + isPurimMeshulash};
    var end = {month: this.hebrewMonths.Adar2, date: 15 + isPurimMeshulash};

    return this._inHebrewHolidayDateRange(begin, end);
  }
  isPurimMeshulash() {
    // Leap years can make Adar2's month number "bounce" so we check for it
    var shushanPurimDate = new HeDate(this.hDate.getFullYear(), this.hebrewMonths.Adar2, 15);
    var isPurimMeshulash = (shushanPurimDate.getDay() === 6);
    var begin = {month: this.hebrewMonths.Adar2, date: 13};
    var end = {month: this.hebrewMonths.Adar2, date: 16};

    return isPurimMeshulash && this._inHebrewHolidayDateRange(begin, end);
  }
  isShvihiShelPesach() {
    // Leap years can make Nisan's month number "bounce" so we check for it
    var begin = {month: this.hebrewMonths.Nisan, date: 20};
    var end = (this.il) ? {month: this.hebrewMonths.Nisan, date: 21} : {month: this.hebrewMonths.Nisan, date: 22};
    return this._inHebrewHolidayDateRange(begin, end);
  }
  isLeapYear() {
...
DeconzAccessory.JewishCalendarSensor = JewishCalendarSensor
