...
class Light extends DeconzAccessory {
  ...
  constructor (gateway, device, settings = {}) {
    ...
    // A temporary workaround to avoid throw exception when a light with Consumption service is set service name to be Switch
    if (this.values.serviceName === 'Switch' && !params.totalConsumptionDelegate) {
      delete params.totalConsumptionDelegate
      delete params.computedConsumptionDelegate
    }
