...
class Gateway extends AccessoryDelegate {
  ...
  async heartbeat (beat) {
    ...
    try {
      ...
      if (beat % putStateEvery === 0) {
        ...
        let keysArray = Object.keys(this.accessoryByRpath)
        if (keysArray.length) {
  ...
  setAqaraS1PanelsConfiguration() {
    ...
    if (this.actionsConfigData && Array.isArray(this.actionsConfigData) === false) {
      ...
      if (aqaraS1Panels) {
        ...
        let devicesSerial = ['6c69676874732f31', '6c69676874732f32', '6c69676874732f33', '6c69676874732f34', '6c69676874732f35', '6375727461696e31', '6375727461696e32', '6375727461696e33', '6169725f636f6e64'] // array of devices serial which is configured when setup is done
              ...
              for (var i = devicesSerial.length - 1; i >= 0; i--) {
                ...
                if (slots) {
                  if (deviceConfig) {
                    ...
                    if (i <= 4) { // Lights
                      // On/Off, general type...
                      commandsToExecute.push('40aa7138468c0201d8024140604f7448' + slots[0].toString(16).padStart(2, '0') + panelSerial + deviceSerial + '04010055' + '260a0' + (deviceConfig.type === 'dim' ? '4' : '5') + '08bfaab9d8d7b4ccac08bfaab9d8d7b4ccac08bfaab9d8d7b4')
                      ...
                      // Brightness
                      commandsToExecute.push('3aaa7134448de0024131604f7448' + slots[1].toString(16).padStart(2, '0') + panelSerial + deviceSerial + '0e010055' + '170a0' + (deviceConfig.type === 'dim' ? '4' : '5') + '0ac1c1b6c8b0d9b7d6b1c8000000000000015' + (parseInt(deviceSerial.charAt(deviceSerial.length-1)) - 1) + '0' + (deviceConfig.type === 'color' ? '2' : '4') + '00')
                      // Name
                      commandsToExecute.push('37aa7131448ee202412e604f7448' + slots[4].toString(16).padStart(2, '0') + panelSerial + deviceSerial + '08001fa5' + '140a0' + (deviceConfig.type === 'dim' ? '4' : '5') + '08c9e8b1b8c3fbb3c60000000000015' + (parseInt(deviceSerial.charAt(deviceSerial.length-1)) - 1) + '0' + (deviceConfig.type === 'color' ? 'a' : 'b') + '00')
                      // Online/Offline
                      commandsToExecute.push('39aa7133448fdf024130604f7448' + slots[5].toString(16).padStart(2, '0') + panelSerial + deviceSerial + '080007fd' + '160a0' + (deviceConfig.type === 'dim' ? '4' : '5') + '0ac9e8b1b8d4dacfdfc0eb0000000000015' + (parseInt(deviceSerial.charAt(deviceSerial.length-1)) - 1) + '3' + (deviceConfig.type === 'color' ? 'c' : 'd') + '00')
                      if (deviceConfig.type === 'ct') {
                        // Color Temperature
                        commandsToExecute.push('36aa713044670a02412d604f7448' + slots[2].toString(16).padStart(2, '0') + panelSerial + deviceSerial + '0e020055' + '130a0506c9abcec2d6b5000000000000015' + (parseInt(deviceSerial.charAt(deviceSerial.length-1)) - 1) + '0300')
                      } else if (deviceConfig.type === 'color') {
                        // Color
                        commandsToExecute.push('36aa713044234e02412d604f7448' + slots[3].toString(16).padStart(2, '0') + panelSerial + deviceSerial + '0e080055' + '130a0506d1d5c9ab7879000000000000015' + (parseInt(deviceSerial.charAt(deviceSerial.length-1)) - 1) + '0100')
                      }
                      ...
                      for (var ii = commandsToExecute.length - 1; ii >= 0; ii--) {
                        ...
                        function executeCommand(index) {
                          ...
                          that.client.put(panel + '/config', {aqara_s1_communication: cmdToSend}).then((obj) => {
                            const sensorId = panel.split('/')[2]
                            that.context.fullState.sensors[sensorId].config.aqara_s1_communication = cmdToSend
                            succeededCommands++
                      ...
                      if (!parsedData.names[i] || parsedData.names[i] !== deviceConfig.name) {
                        ...
                        function executeCommand(index) {
                          ...
                          that.client.put(panel + '/config', {aqara_s1_communication: dataToSend}).then((obj) => {
                            const sensorId = panel.split('/')[2]
                            that.context.fullState.sensors[sensorId].config.aqara_s1_communication = dataToSend
  ...
  processIncomingDataFromAqaraS1(updateData, rpath) {
    ...
    let sum = dataArray[dataStartIndex] + dataArray[dataStartIndex + 1] + dataArray[dataStartIndex + 2] + dataArray[dataStartIndex + 3] + dataArray[dataStartIndex + 4] + this.getInt8(dataArray[dataStartIndex + 5])
    this.debug('Data array: ' + dataArray + ', Integrity: ' + dataArray[dataStartIndex + 5] + ', Signed integrity: ' + this.getInt8(dataArray[dataStartIndex + 5]) + ', Sum: ' + sum)

    if (sum === 512 && dataArray.length >= dataStartIndex + 20 - 1) {
      ...
      let commandType = dataArray[dataStartIndex + 3] // 84=Attribute report of states, 24=ACK for state commands, 44 commands for device (shouldn't happen here), 46=multi-part commands for device (also shouldn't happen here)
      let commandAction = dataArray[dataStartIndex + 6] // 1=state report/scenes config, 2=configs, 3=scenes activation, 4=removals, 5=set state/states ACKs, 6=state request
      let paramsSize = dataArray[dataStartIndex + 8]
      let deviceSerial = commandType === 0x24 ? [dataArray[dataStartIndex + 10], dataArray[dataStartIndex + 11], dataArray[dataStartIndex + 12], dataArray[dataStartIndex + 13], dataArray[dataStartIndex + 14], dataArray[dataStartIndex + 15], dataArray[dataStartIndex + 16], dataArray[dataStartIndex + 17]] : [dataArray[dataStartIndex + 9], dataArray[dataStartIndex + 10], dataArray[dataStartIndex + 11], dataArray[dataStartIndex + 12], dataArray[dataStartIndex + 13], dataArray[dataStartIndex + 14], dataArray[dataStartIndex + 15], dataArray[dataStartIndex + 16]]
      let stateParam = commandType === 0x24 ? [] : [dataArray[dataStartIndex + 17], dataArray[dataStartIndex + 18], dataArray[dataStartIndex + 19], dataArray[dataStartIndex + 20]]
      ...
      if (commandCategory === 0x72 && commandType === 0x84 && commandAction === 0x01 && this.platform.platformAccessory.service.values.switchesOn) { // State of device is reported.
      ...
      } else if (commandCategory === 0x71 && commandType === 0x84 && commandAction === 0x06) { // Panel asking for data...
        ...
        if (stateParam[0] === 0x08 && stateParam[1] === 0x00 && stateParam[2] === 0x1f && stateParam[3] === 0xa5) { // Names
          if (this.actionsConfigData.aqara_S1_panels[rpath]) {
            // console.log(this.actionsConfigData.aqara_S1_panels[rpath])
            // console.log(deviceResourceType === 'lights/' ? 'light_' + deviceResourceType.charAt(deviceResourceType.length-1) : deviceResourceType === 'curtain' ? 'curtain_' + deviceResourceType.charAt(deviceResourceType.length-1) : 'ac')
          ...
          this.sendStateToPanel(('/' + this.id + rpath).split('/'), deviceSerialStr, '080007fd', '00000001')
          ...
          if (stateParam[0] === 0x01 && stateParam[1] === 0x01 && stateParam[2] === 0x00 && stateParam[3] === 0x55) { // Position
            ...
            if (accessoryToControl) {
              ...
              this.sendStateToPanel(('/' + this.id + rpath).split('/'), deviceSerialStr, '01010055', this.getAqaraHexFromInt(100).toString(16).padStart(4, '0') + '0000')
            }
          } else if (stateParam[0] === 0x0e && stateParam[1] === 0x02 && stateParam[2] === 0x00 && stateParam[3] === 0x55) { // Position State
            const panelDevicePath = '/' + this.id + rpath + '/curtain_' + deviceResourceType.charAt(deviceResourceType.length-1)
            const pathComponents = this.platform.panelsToResources[panelDevicePath][0].split( '/' )
            ...
            if (accessoryToControl) {
              // TODO: Send soemthing, maybe just stationary state (Paused)...
              // accessoryToControl.service.updatePanelPositionState(panelDevicePath.split('/'))
            } else {
        ...
        } else if (dataArray[dataStartIndex + 9] === 0x00 && (!this.actionsConfigData.aqara_S1_panels[rpath] || (deviceResourceType.startsWith('lights/') && !this.actionsConfigData.aqara_S1_panels[rpath]['light_' + deviceResourceType.charAt(deviceResourceType.length-1)]) || (deviceResourceType.startsWith('curtain') && !this.actionsConfigData.aqara_S1_panels[rpath]['curtain_' + deviceResourceType.charAt(deviceResourceType.length-1)]) || (deviceResourceType === 'air_cond' && !this.actionsConfigData.aqara_S1_panels[rpath]['ac']))) { // A device is set on the device, but shouldn't be there (removed from config...)

        }
      } else if (commandCategory === 0x71 && commandType === 0x84 && commandAction === 0x07) { // ACKs for configuration commmands...
        this.log('Configuration received...')
      } else {
        this.log('Unknown message: commandCategory: 0x' + commandCategory.toString(16) + ', commandType: 0x' + commandType.toString(16) + ', commandAction: 0x' + commandAction.toString(16) + ', paramsSize: 0x' + paramsSize.toString(16) + ', deviceSerial: ' + deviceSerial + ', stateParam: ' + stateParam)
      }
    } else {
      this.log('Data array: ' + dataArray + ', Integrity: ' + dataArray[dataStartIndex + 5] + ', Signed integrity: ' + this.getInt8(dataArray[dataStartIndex + 5]) + ', Sum: ' + sum)
    }
  }
      ...
      .on('changed', (rtype, rid, body) => {
        try {
          ...
          if (rtype === 'sensors' && body.state !== undefined && body.state.buttonevent !== undefined && this.actionsConfigData && this.actionsConfigData[rid] && this.actionsConfigData[rid].length && this.platform.platformAccessory.service.values.switchesOn) {
              ...
              if (body.state.buttonevent === 2001 || body.state.buttonevent === 3001 || body.state.buttonevent === 4001 || body.state.buttonevent === 5001) {
                ...
                if (actionConfig.resourcePath && actionConfig.resourcePath.startsWith("/")) {
                  ...
                  if (accessoryToControl) {
                    ...
                    } else if (((sensorTypeInt == 0 && body.state.buttonevent === 2002) || (sensorTypeInt == 1 && body.state.buttonevent === 2000)) && service.testCharacteristic(this.platform.Characteristics.hap.Brightness)) {
                      if (!service.getCharacteristic(this.platform.Characteristics.hap.On).value) {
                        service.getCharacteristic(this.platform.Characteristics.hap.Brightness).setValue(1);
                        service.getCharacteristic(this.platform.Characteristics.hap.On).setValue(true);
                      } else {
                    ...
                    } else if (((sensorTypeInt == 0 && body.state.buttonevent === 3002) || (sensorTypeInt == 1 && body.state.buttonevent === 3000)) && service.testCharacteristic(this.platform.Characteristics.hap.Brightness)) {
                      var characteristic = service.getCharacteristic(this.platform.Characteristics.hap.Brightness);
                      var newBrightnessState = Math.max(1, characteristic.value - 5);
                      characteristic.setValue(newBrightnessState);
            ...
            if (rtype === 'sensors' && body.config !== undefined && body.config['aqara_s1_communication'] !== undefined && body.config['aqara_s1_communication'] !== this.context.fullState[rtype][rid].config.aqara_s1_communication) {
