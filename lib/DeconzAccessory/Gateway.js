    ...
    if (sum === 512 || sum === 256 || sum === 768) {
      if (commandType === 0x84) {
        ...
        if (commandCategory === 0x71 && commandAction === 0x02) { // ACKs for confguration commmands...
          ...
          if (dataArray[dataStartIndex + 9] === 0x01) { // A device is missing... (We send a state to unconfigured device, for example, we sent a light on/off state for light_1 while it isn't configured on the panel, so, we should (re))configure it...
          ...
          } else if (dataArray[dataStartIndex + 9] === 0x00 && (!this.actionsConfigData.aqara_S1_panels[rpath] || ((dataArray[dataStartIndex + 18] === 0x01 || dataArray[dataStartIndex + 18] === 0x02) && !this.actionsConfigData.aqara_S1_panels[rpath]['temperature_sensor']))) { // A device is set on the device, but shouldn't be there (removed from config...)
