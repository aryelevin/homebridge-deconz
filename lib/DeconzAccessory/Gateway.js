...
class Gateway extends AccessoryDelegate {
  ...
  async heartbeat (beat) {
    ...
    try {
      ...
      // Added by me: Arye Levin
      if (!this.platform._configJson.homeassistantMode) {
          ...
          if (stateParam[0] === 0x08 && stateParam[1] === 0x00 && stateParam[2] === 0x1f && stateParam[3] === 0xa5) { // Names
            ...
            if (stateParam[0] === 0x01 && stateParam[1] === 0x01 && stateParam[2] === 0x00 && stateParam[3] === 0x55) { // Position
              ...
              if (accessoryToControl) {
                ...
                this.sendStateToPanel(('/' + this.id + rpath).split('/'), deviceSerialStr, '01010055', this.getHexFromFloat32Bit(0).toString(16).padStart(4, '0') + '0000')
              }
            } else if (stateParam[0] === 0x0e && stateParam[1] === 0x02 && stateParam[2] === 0x00 && stateParam[3] === 0x55) { // Position State
              const panelDevicePath = '/' + this.id + rpath + '/curtain_' + deviceResourceType.charAt(deviceResourceType.length - 1)
              const pathComponents = this.platform.panelsToResources[panelDevicePath][0].split('/')
  ...
  processIncomingButtonEvent (rid, buttonevent) {
    const actionsConfig = this.actionsConfigData['/' + this.id + '/sensors/' + rid]
        ...
        if (accessoryToControl) {
          ...
          function repeatZBFunction (delay, timeoutKey) {
            longPressTimeoutIDs[timeoutKey] = setTimeout(function () {
              ...
              if (actionToDo.startsWith('on_low_bri')) {
              ...
              } else if (actionToDo.startsWith('toogle_on')) {
                ...
                if (actionToDo === 'toogle_on_full_bri' && newPowerState && service.testCharacteristic(that.platform.Characteristics.hap.Brightness)) {
    ...
    this.wsClient
      ...
      .on('changed', (rtype, rid, body) => {
        try {
          ...
          // Added by me: Arye Levin
          if (!this.platform._configJson.homeassistantMode && this.initialised) {
            if (rtype === 'sensors' && body.state !== undefined && body.state.buttonevent !== undefined && this.actionsConfigData && (this.actionsConfigData['/' + this.id + rpath] || (this.actionsConfigData[rid] && this.actionsConfigData[rid].length)) && this.platform.platformAccessory.service.values.switchesOn) {
              ...
              if (this.platform.panelsToResources['/' + this.id + rpath + '/switch'] !== undefined && this.platform.platformAccessory.service.values.switchesOn) {
                ...
                for (let i = resources.length - 1; i >= 0; i--) {
                  const resourceItem = resources[i]
                  const pathComponents = resourceItem.split('/')
                  const controlledRPath = '/' + pathComponents[2] + '/' + pathComponents[3]
                  const accessoryToControl = this.platform.gatewayMap[pathComponents[1]].accessoryByRpath[controlledRPath]

                  if (accessoryToControl) {
                    accessoryToControl.serviceByRpath[controlledRPath]._service.getCharacteristic(this.platform.Characteristics.hap.On).setValue(body.state.on)
                  }
                }
              }
            }
