...
class Gateway extends AccessoryDelegate {
                  ...
                  if (deviceConfig) {
                    ...
                    if (parsedData[i]) {
                      ...
                      for (var ii = slots.length - 1; ii >= 0; ii--) {
                        commandsToExecute.push('22aa711c4498ed0441196044f0ed' + slots[ii].toString(16).padStart(2, '0') + panelSerial + '000000000000000000000000')
                      }
                      this.configurationCommandsToExecute.push({commands: commandsToExecute,
                                                                meta: { index: 0, 
                                                                        deviceIndex: i, 
      ...
      if (currentConfiguredDeviceCommandsArray !== undefined) {
        if (currentConfiguredDeviceCommandsArray.meta.index + 1 === currentConfiguredDeviceCommandsArray.commands.length || currentConfiguredDeviceCommandsArray.meta.failureCount >= 3) {
          if (currentConfiguredDeviceCommandsArray.meta.failureCount < 3) {
            ...
            this.log('Going to write sent commands for deviceIndex: ' + deviceIndex + '.')
  ...
  processIncomingDataFromAqaraS1(updateData, rpath) {
    ...
    let commandAction = dataArray[commandActionByteIndex] // 1=state report/scenes config, 2=configs, 3=scenes activation, 4=removals, 5=set state/states ACKs, 6=state request
    ...
    if (sum === 512 || sum === 256 || sum === 768) {
      if (commandType === 0x84) {
        ...
        let deviceSerial = [dataArray[dataStartIndex + 9], dataArray[dataStartIndex + 10], dataArray[dataStartIndex + 11], dataArray[dataStartIndex + 12], dataArray[dataStartIndex + 13], dataArray[dataStartIndex + 14], dataArray[dataStartIndex + 15], dataArray[dataStartIndex + 16]]
        let stateParam = [dataArray[dataStartIndex + 17], dataArray[dataStartIndex + 18], dataArray[dataStartIndex + 19], dataArray[dataStartIndex + 20]]
        ...
        if (commandCategory === 0x72 && commandAction === 0x01) { // State of device is reported.
          ...
          if (stateParam[0] === 0x08 && stateParam[1] === 0x00 && stateParam[2] === 0x1f && stateParam[3] === 0xa5) { // Names
            if (this.actionsConfigData.aqara_S1_panels[rpath]) {
              ...
              // console.log(deviceResourceType.startsWith('lights/') ? 'light_' + deviceResourceType.charAt(deviceResourceType.length-1) : deviceResourceType.startsWith('curtain') ? 'curtain_' + deviceResourceType.charAt(deviceResourceType.length-1) : 'ac')
        ...
        } else if (commandCategory === 0x71 && commandAction === 0x07) { // TODO: check what it is, it seems to be ACKs for configuration commmands somehow, not sure yet...
          this.log('Configuration received...')
        } else {
          this.log('Unknown message: Data array: ' + dataArray + ', Integrity: ' + dataArray[integrityByteIndex] + ', Signed integrity: ' + this.getInt8(dataArray[integrityByteIndex]) + ', Sum: ' + sum + ', commandCategory: 0x' + commandCategory.toString(16) + ', commandType: 0x' + commandType.toString(16) + ', commandAction: 0x' + commandAction.toString(16) + ', paramsSize: 0x' + paramsSize.toString(16))
        }
      } else if (commandType === 0x24) {
        let paramsSize = dataArray[dataStartIndex + 8]
        
        if (commandCategory === 0x71 && commandAction === 0x02) { // ACKs for confguration commmands...
          this.log('Configuration command ACK...')
          this.executeNextConfigurationCommand()
        } else if (commandCategory === 0x71 && commandAction === 0x04) { // ACKs for confguration removal commmands...
        ...
        if (commandCategory === 0x71 && commandAction === 0x02) {
          ...
          if (partNo < totalParts) {
            this.executeNextConfigurationCommand() // TODO: check the part no and compare it to the current index of the current configuration commands set...
          } else {
            // We should receive now commandType === 0x24 && commandCategory === 0x71 && commandAction === 0x02 above...
          }
        }
      } else {
        this.log('Unknown message: Data array: ' + dataArray + ', Integrity: ' + dataArray[integrityByteIndex] + ', Signed integrity: ' + this.getInt8(dataArray[integrityByteIndex]) + ', Sum: ' + sum + ', commandCategory: 0x' + commandCategory.toString(16) + ', commandType: 0x' + commandType.toString(16) + ', commandAction: 0x' + commandAction.toString(16))
      }
    } else {
      this.log('Unknown message: Data array: ' + dataArray + ', Integrity: ' + dataArray[integrityByteIndex] + ', Signed integrity: ' + this.getInt8(dataArray[integrityByteIndex]) + ', Sum: ' + sum + ', commandCategory: 0x' + commandCategory.toString(16) + ', commandType: 0x' + commandType.toString(16) + ', commandAction: 0x' + commandAction.toString(16))
    }
