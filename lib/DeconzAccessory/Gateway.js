              ...
              if (sum === 512) {
                ...
                if (deviceResourceType) {
                  if (commandCategory === 0x72 && commandType === 0x84 && commandAction === 0x01 && this.platform.platformAccessory.service.values.switchesOn) { // State of device is reported.
                    ...
                    if (deviceResourceType === 'air_cond' && stateParam[0] === 0x0e && stateParam[2] === 0x00 && stateParam[3] === 0x55 && (stateParam[1] === 0x20 || stateParam[1] === 0x02)) { // Updated Air conditioner/Heater-Cooler device state
                      const panelDevicePath = '/' + this.id + '/' + rtype + '/' + rid + '/ac'
                      ...
                      const panelDevicePath = '/' + this.id + '/' + rtype + '/' + rid + '/curtain_' + deviceSerial[7]
                      const pathComponents = this.platform.panelsToResources[panelDevicePath][0].split( '/' )
                      const accessoryToControl = this.platform.gatewayMap[pathComponents[1]].accessoryByRpath['/' + pathComponents[2] + '/' + pathComponents[3]]
                      ...
                      if (accessoryToControl && accessoryToControl.service._service.testCharacteristic(this.platform.Characteristics.hap.TargetPosition)) {
                        ...
                        const that = this
                        const dataToSend = '19aa7113446d210541106375727461696e' + deviceSerial[7].toString(16).padStart(2, '0') + '01010055' + position + '0000'
                        
                        this.client.put(rpath + '/config', {aqara_s1_communication: dataToSend}).then((obj) => {
                          that.context.fullState[rtype][rid].config.aqara_s1_communication = dataToSend
                        }).catch((error) => {
                          
                        })
                      }
                    } else if (deviceResourceType === 'lights/' && stateParam[2] === 0x00 && stateParam[3] === 0x55) {
                      const panelDevicePath = '/' + this.id + '/' + rtype + '/' + rid + '/light_' + deviceSerial[7]
                      const pathComponents = this.platform.panelsToResources[panelDevicePath][0].split( '/' )
                      const accessoryToControl = this.platform.gatewayMap[pathComponents[1]].accessoryByRpath['/' + pathComponents[2] + '/' + pathComponents[3]]
    
                      if (accessoryToControl && accessoryToControl.values.serviceName === 'Light') {
                        if (stateParam[0] === 0x04 && stateParam[1] === 0x01) { // Light On/Off
                          accessoryToControl.service.updatePanelOnOffState(panelDevicePath.split('/'))
                        } else if (stateParam[0] === 0x0e && stateParam[1] === 0x01) { // Light Brightness
                          accessoryToControl.service.updatePanelBrightnessState(panelDevicePath.split('/'))
                        } else if (stateParam[0] === 0x0e && stateParam[1] === 0x02) { // Light CT
                          accessoryToControl.service.updatePanelColorTemperatureState(panelDevicePath.split('/'))
                        } else if (stateParam[0] === 0x0e && stateParam[1] === 0x08) { // Light Color
                      ...
                      const name = this.actionsConfigData.aqara_S1_panels['/' + rtype + '/' + rid][deviceResourceType === 'lights/' ? 'light_' + deviceSerial[7] : deviceResourceType === 'curtain' ? 'curtain_' + deviceSerial[7] : 'ac'].name
    
                      const nameSize = name.length
                      const nameHex = toHexString(name.split ('').map (function (c) { return c.charCodeAt (0); }))
                      ...
                      const that = this
                      const dataToSend = '19aa7113446d21054110' + (deviceResourceType === 'lights/' ? ('6c69676874732f' + deviceSerial[7].toString(16).padStart(2, '0')) : deviceResourceType === 'curtain' ? ('6375727461696e' + deviceSerial[7].toString(16).padStart(2, '0')) : '6169725f636f6e64') + '080007fd' + '00000001'
