      ...
      for (var i = panelsToUpdate.length - 1; i >= 0; i--) {
        ...
        if (pathComponents[4].startsWith('curtain')) {
          ...
          if (shouldUpdatePanelState) {
            ...
            this.log('Going to set "' + '19aa7113446d21054110' + '6375727461696e' + parseInt(pathComponents[4].charAt(pathComponents[4].length-1)).toString(16).padStart(2, '0') + parameter + content + '" at panel: ' + panelResourcePath);
            let that = this
            this.gateway.platform.gatewayMap[pathComponents[1]].client.put(panelResourcePath, {aqara_s1_panel_communication: '19aa7113446d21054110' + '6375727461696e' + parseInt(pathComponents[4].charAt(pathComponents[4].length-1)).toString(16).padStart(2, '0') + parameter + content}).then((obj) => {
              that.log('Successfully set "' + '19aa7113446d21054110' + '6375727461696e' + parseInt(pathComponents[4].charAt(pathComponents[4].length-1)).toString(16).padStart(2, '0') + parameter + content + '" at panel: ' + panelResourcePath);
            }).catch((error) => {
