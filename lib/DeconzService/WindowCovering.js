...
class WindowCovering extends DeconzService.LightsResource {
  constructor (accessory, resource, params = {}) {
    ...
    // Added by me: Arye Levin
      ...
      // TODO: maybe i should set the this.values.positionState = this.Characteristics.hap.PositionState.STOPPED here, as this means the movement finished... (maybe compare target and current position before doing so... only if the're equal set it...)
        this.log('Current position set to %d while the state is %s: ', value, this.values.positionState === this.Characteristics.hap.PositionState.INCREASING ? 'INCREASING' : this.values.positionState === this.Characteristics.hap.PositionState.DECREASING ? 'DECREASING' : 'STOPPED')
        ...
        // Update it only when stopped, since while moving the screen just animates the curtain opening, so avoid stopping it with this call...
        // Also update if the current and target position is same since it means it finished operation and might be not yet updated the PositionState value.
        if (this.values.positionState === this.Characteristics.hap.PositionState.STOPPED || value === this.values.targetPosition) {
          this.updatePanels('01010055', this.gateway.getHexFromFloat32Bit(value), 'targetPosition')
        }
        this.updatePanels('0e020055', '00000002', 'targetPosition') // Stopped (Is it needed??)
      })
  ...
  updateState (state) {
    if (state.lift != null) {
      ...
      if (
        // Added by me: Arye Levin
        // In case a device update position after it stopped externally (for example: by remote control or Aqara S1 Scene panel)
        this.values.positionState === this.Characteristics.hap.PositionState.STOPPED ||
        // End of Added by me: Arye Levin
  ...
  updatePanelPositionState (pathComponents) {
    const aqaraPosition = this.gateway.getHexFromFloat32Bit(this.values.targetPosition)
    this.log('HK Position: ' + this.values.targetPosition + ', Aqara Position: ' + aqaraPosition)
    this.updatePanel(pathComponents, '01010055', aqaraPosition)
  }
