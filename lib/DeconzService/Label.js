...
class Label extends DeconzService.SensorsResource {
  ...
  updateState (state, rpath) {
    ...
    if (!buttonResource || buttonResource.lastUpdated === '') {
      if (buttonResource) {
  ...
  putState() {
    // Nothing to do (This just added for handling the 'lumi.switch.n4acn4' model type of device, since it sets the primary service to be a Label vs. Light since we can configure which lights is exposed and which isn't)
  }
}
