...
class Platform extends ServiceDelegate {
  constructor (platformAccessory, params = {}) {
    ...
    if (!this.platformAccessory.platform._configJson.homeassistantMode) {
      this.addCharacteristicDelegate({
        ...
        if (fromHomeKit) {
          ...
          // Commented since i prefer not to update shabbat mode by turning it on/off, only shbbat mode controls it, not the opposite...
          ...
          // }
          // const shabbatModeCLIPSensor = this.platformAccessory.platform._configJson.shabbatModeCLIPSensor
          // if (shabbatModeCLIPSensor !== undefined) {
          //   const pathComponents = shabbatModeCLIPSensor.split("/")
          //   this.platform.gatewayMap[pathComponents[1]]?.accessoryByType[pathComponents[2]]?.servicesById[pathComponents[3]]?._service.getCharacteristic()
          // }
        }
        const commandsToExecute = value ? this.platformAccessory.platform._configJson.switchesOnStateCommands : this.platformAccessory.platform._configJson.switchesOffStateCommands
