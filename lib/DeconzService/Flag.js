...
class Flag extends DeconzService.SensorsResource {
  ...
  updateState (state) {
    if (state.flag != null) {
      ...
      if (this.platform._configJson.shabbatModeCLIPSensor === '/' + this.gateway.id + this.resource.rpath && state.flag === this.platformAccessory.service.characteristicDelegate('switchesOn').value) {
        this.platformAccessory.service.characteristicDelegate('switchesOn').value = !state.flag
      }
