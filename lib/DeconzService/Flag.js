...
class Flag extends DeconzService.SensorsResource {
  ...
  updateState (state) {
    if (state.flag != null) {
      ...
      if (!this.gateway.platform._configJson.homeassistantMode && this.gateway.platform._configJson.shabbatModeCLIPSensor === '/' + this.gateway.id + this.resource.rpath && state.flag === this.gateway.platform.platformAccessory.service.characteristicDelegate('switchesOn').value) {
        this.gateway.platform.platformAccessory.service.characteristicDelegate('switchesOn').value = !state.flag
      }
