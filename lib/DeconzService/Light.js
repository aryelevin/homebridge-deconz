...
class Light extends DeconzService.LightsResource {
  ...
  updateState (state, rpath, stateKey = 'state') {
    if (this.resource.manufacturer !== 'ubisys' && this.resource.manufacturer !== 'EcoDim B.V' && this.resource.manufacturer !== 'ROBB smarrt' && this.resource.manufacturer !== 'AduroSmart Eria' && this.resource.manufacturer !== 'SmartDimmer' && this.resource.manufacturer !== 'Idinio' && this.resource.model !== 'lumi.switch.n4acn4') {
      ...
      if (state.on !== undefined && state.on != this.values.on) {
        const persistedValue = this.values.on
        this.values.on = state.on // This is to allow the setValue() below to function (it checks first if there is a change of the value)
        state.on = persistedValue
        this._service.getCharacteristic(this.platform.Characteristics.hap.On).setValue(state.on)
      }
      if (state.all_on !== undefined && state.all_on != this.values.on) {
        const persistedValue = this.values.on
        this.values.on = state.all_on // This is to allow the setValue() below to function (it checks first if there is a change of the value)
        state.all_on = persistedValue
        this._service.getCharacteristic(this.platform.Characteristics.hap.On).setValue(state.all_on)
      }
      if (state.bri !== undefined) {
        const brightness = Math.round(this.values.brightness * 2.54)
        if (state.bri != brightness) {
          const persistedValue = this.values.brightness
          this.values.brightness = Math.max(1, Math.round(state.bri / 2.54)) // This is to allow the setValue() below to function (it checks first if there is a change of the value)
          state.bri = brightness
          this._service.getCharacteristic(this.platform.Characteristics.hap.Brightness).setValue(persistedValue)
        }
      }
      // if (state.ct !== undefined && state.ct != this.values.colorTemperature) {
      //   state.ct = this.values.colorTemperature
      //   this._service.getCharacteristic(this.platform.Characteristics.hap.ColorTemperature).setValue(state.ct)
      // }
    }
    this.initAdaptiveLighting()
  ...
  putState () {
    // if ((this.config.doorLock && this.service.getCharacteristic(Characteristic.LockCurrentState).value == this.service.getCharacteristic(Characteristic.LockTargetState).value)) {
    //   // this.log('putState: %s is most probably a warning device such as a smoke sensor, skipping...', this.resource.rpath)
    //   return
    // }
    ...
    var params = { on: newOn ? true : false }
    ...
    if (this.capabilities.ct && this.values.colormode === 'ct' && newOn && this.values.colorTemperature) {
      ...
      let newCt = this.values.colorTemperature
      ...
      for (var i = panelsToUpdate.length - 1; i >= 0; i--) {
        ...
        if (pathComponents[4].startsWith('light')) {
          ...
          for (var ii = lightsControlledWithPanelDevice.length - 1; ii >= 0; ii--) {
            ...
            let accessoryToCheck = this.gateway.platform.gatewayMap[lightResourcePath[1]].accessoryByRpath['/' + lightResourcePath[2] + '/' + lightResourcePath[3]].service
            if (accessoryToCheck) {
        ...
        for (var i = panelsToUpdate.length - 1; i >= 0; i--) {
          ...
          if (pathComponents[4] === 'switch'/* && that.bridge.platform.bridgeMap[pathComponents[1]].fullState.lights[pathComponents[3]].state.on != that.hk.on*/) {
            ...
            if (newOn) {
              ...
              for (var ii = lightsControlledWithPanelDevice.length - 1; ii >= 0; ii--) {
                let lightResourcePath = lightsControlledWithPanelDevice[ii].split('/')
                let accessoryToCheck = that.gateway.platform.gatewayMap[lightResourcePath[1]].accessoryByRpath['/' + lightResourcePath[2] + '/' + lightResourcePath[3]].service
                if (accessoryToCheck) {
