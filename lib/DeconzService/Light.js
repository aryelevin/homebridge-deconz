...
class Light extends DeconzService.LightsResource {
  ...
  updateState (state, rpath, stateKey = 'state') {
    // Added by me: Arye Levin
    if (!this.platform._configJson.homeassistantMode && ((this.platform._configJson.resourcesToIgnoreDeviceStateUpdates !== undefined && this.platform._configJson.resourcesToIgnoreDeviceStateUpdates?.includes('/' + this.gateway.id + this.resource.rpath)) || (this.platform._configJson.shabbatModeCLIPSensor !== undefined && !this.platform.platformAccessory.service.values.switchesOn))) {
