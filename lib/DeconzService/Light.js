...
class Light extends DeconzService.LightsResource {
  constructor (accessory, resource, params = {}) {
    ...
    if (this.capabilities.bri) {
      this.brightnessDelegate = this.addCharacteristicDelegate({
        ...
        if (fromHomeKit) {
          if (this.gateway.platform._configJson.useHTTPRequestsForPUTWithPort) { 
            this.platform.platformAccessory.sendPUTRequestToServer(this.gateway.id, this.accessory.id, this.resource.subtype, 'brightness', value)
          } else {
    ...
    if (this.capabilities.ct) {
      ...
      this.colorTemperatureDelegate = this.addCharacteristicDelegate({
        ...
        if (fromHomeKit) {
          if (this.gateway.platform._configJson.useHTTPRequestsForPUTWithPort) { 
            this.platform.platformAccessory.sendPUTRequestToServer(this.gateway.id, this.accessory.id, this.resource.subtype, 'colorTemperature', value)
          } else {
            this.checkAdaptiveLighting()
            // Added by me: Arye Levin
            // this.putState({ ct })
  ...
  updateState (state, rpath, stateKey = 'state') {
    // Added by me: Arye Levin
    if (this.platform._configJson.excludeForceState !== undefined && /* Fingerbot -> */!(this.resource.manufacturer === '_TZ3210_dse8ogfy' && this.resource.model === 'TS0001') && this.resource.manufacturer !== 'ubisys' && this.resource.manufacturer !== 'EcoDim B.V' && this.resource.manufacturer !== 'ROBB smarrt' && this.resource.manufacturer !== 'SmartDimmer' && this.resource.manufacturer !== 'Idinio' && (this.resource.model !== 'lumi.switch.n4acn4' || !this.platform.platformAccessory.service.values.switchesOn) && (this.resource.model !== 'lumi.switch.n1acn1' || !this.platform.platformAccessory.service.values.switchesOn) && (this.resource.model !== 'lumi.switch.n2acn1' || !this.platform.platformAccessory.service.values.switchesOn) && (this.resource.model !== 'lumi.switch.n3acn1' || !this.platform.platformAccessory.service.values.switchesOn) && !this.platform._configJson.excludeForceState?.includes('/' + this.gateway.id + this.resource.rpath)) {
