...
class HeaterCooler extends DeconzService.SensorsResource {
  constructor (accessory, resource, params = {}) {
    ...
    this.addCharacteristicDelegate({
      ...
      if (fromHomeKit) {
        await this.put('/config', resource.body.config.coolsetpoint !== undefined ? { coolsetpoint: Math.round(value * 100) } : { temperature: Math.round(value * 100) })
    ...
    this.addCharacteristicDelegate({
      ...
      if (fromHomeKit) {
        await this.put('/config', resource.body.config.heatsetpoint !== undefined ? { heatsetpoint: Math.round(value * 100) } : { temperature: Math.round(value * 100) })
  ...
  updateConfig (config) {
    ...
    if (config.temperature != null) {
      this.values.heatingThresholdTemperature = Math.round(config.temperature / 50) / 2
      this.values.coolingThresholdTemperature = Math.round(config.temperature / 50) / 2
    }
