...
class HeaterCooler extends DeconzService.SensorsResource {
  ...
  updatePanels(hkValueToCheck) {
    ...
    if (panelsToUpdate && Array.isArray(panelsToUpdate) && panelsToUpdate.length) {
      for (var i = panelsToUpdate.length - 1; i >= 0; i--) {
        ...
        let panelResourcePathComponents = panelResourceItem.split( '/' )
        if (panelResourcePathComponents[4] === 'ac') {
          ...
          if (hkValueToCheck) {
            ...
            for (var ii = acsControlledWithPanelDevice.length - 1; ii >= 0; ii--) {
              ...
              let accessoryToCheck = this.gateway.platform.gatewayMap[acResourcePath[1]].accessoryByRpath['/' + acResourcePath[2] + '/' + acResourcePath[3]]
              if (accessoryToCheck) {
                if (accessoryToCheck !== this && accessoryToCheck.service.values[hkValueToCheck] !== undefined && accessoryToCheck.service.values[hkValueToCheck] != this.values[hkValueToCheck]) {
          ...
          if (shouldUpdatePanelState) {
            this.updatePanel(panelResourcePathComponents)
          }
        }
      }
    }
  ...
  updatePanel(panelResourcePathComponents) {
    let internalThermostat = this.gateway.platform._configJson.actionsConfigData[panelResourcePathComponents[1]].aqara_S1_panels['/' + panelResourcePathComponents[2] + '/' + panelResourcePathComponents[3]].ac.internal_thermostat
    let temperature = this.values.targetHeaterCoolerState === this.Characteristics.hap.TargetHeaterCoolerState.HEAT ? this.values.heatingThresholdTemperature : this.values.targetHeaterCoolerState === this.Characteristics.hap.TargetHeaterCoolerState.COOL ? this.values.coolingThresholdTemperature : 255

    let newState = (internalThermostat ? '0e020055' : '0e200055') + (this.values.active === this.Characteristics.hap.Active.ACTIVE ? '1' : '0') + (this.values.targetHeaterCoolerState === this.Characteristics.hap.TargetHeaterCoolerState.HEAT ? '0' : this.values.targetHeaterCoolerState === this.Characteristics.hap.TargetHeaterCoolerState.COOL ? '1' : '2') + (this.values.fanmode === 25 ? '0' : this.values.fanmode === 50 ? '1' : this.values.fanmode === 75 ? '2' : '3') + (internalThermostat ? '0' : 'f') + temperature.toString(16).padStart(2, '0') + (internalThermostat ? ((Math.round(this.values.currentTemperature) + 0) * 4).toString(16).padStart(2, '0') : '00')

    let panelResourcePath = '/' + panelResourcePathComponents[2] + '/' + panelResourcePathComponents[3] + '/config'
    const that = this
    this.gateway.platform.gatewayMap[panelResourcePathComponents[1]].client.put(panelResourcePath, {aqara_s1_communication: '19aa7113446d21054110' + '6169725f636f6e64' + newState}).then((obj) => {
      that.log('Successfully set "' + '19aa7113446d21054110' + '6169725f636f6e64' + newState + '" at panel: ' + panelResourcePath);
    }).catch((error) => {
